# LLMorpheus (thesis fork: `llmorpheus-neo`)

This tree is a **copy of the paper LLMorpheus** used for thesis experiments. It adds optional **Langdock multi-provider** support: pass `--langdock` or set `LLMORPHEUS_LANGDOCK=1`. **OpenAI-style models** (e.g. `gpt-4o-mini`) use Langdock’s OpenAI-compatible client (`/openai/{region}/v1/chat/completions`). Model ids containing `claude` or `gemini` use Langdock’s Anthropic or Google routes. Set `LLMORPHEUS_LLM_AUTH_HEADERS` (JSON), and optionally `LLMORPHEUS_LANGDOCK_BASE_URL` (default `https://api.langdock.com`) and `LLMORPHEUS_LANGDOCK_REGION` (`eu` or `us`, default `eu`). The upstream copy lives under `source/llmorpheus` in the main repo.
