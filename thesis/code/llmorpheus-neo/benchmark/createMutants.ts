...
import { LangdockModel } from "../src/model/LangdockModel";
...
import { langdockEnabledFromEnv } from "../src/model/langdockRoutes";
...
if (require.main === module) {
  (async () => {
    const parser = yargs(hideBin(process.argv))
      ...
      .options({
        ...
        langdock: {
          ...
          demandOption: false,
          description:
            "use Langdock for all models: GPT/OpenAI-style ids use Langdock's OpenAI client (/openai/{region}/v1/chat/completions); claude→Anthropic, gemini→Google. Same as env LLMORPHEUS_LANGDOCK=1. Requires LLMORPHEUS_LLM_AUTH_HEADERS; optional LLMORPHEUS_LANGDOCK_BASE_URL, LLMORPHEUS_LANGDOCK_REGION",
        },
      });
    ...
    if (argv.replay !== undefined) {
      ...
      const useLangdock = argv.langdock || langdockEnabledFromEnv();
      const baseModel = useLangdock
        ? new LangdockModel(
            argv.model,
